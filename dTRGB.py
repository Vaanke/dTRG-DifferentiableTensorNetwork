...
class dHOTRG(torch.nn.Module):
    ...
    def calc_transferM(self, U0, U1, T0, T1):
        # T:     2               U:
        #        |                    1 ---|
        #    1 ----- 3                     |---3
        #        |                    2 ---|
        #        4
        M1 = ncon([U0,T0,U0,T1,U1,U1],[[-1,1,-5],  [1,-4,3,2],  [-3,3,4],  [4,6,-7,5], [5,2,-8],  [6,-2,-6]])
        M2 = ncon([U0,T0,U0,T1,U1,U1],[[ 1,-1,-5], [1,2,3,-2],  [3,-3,4],  [4,6,-7,5], [5,-4,-8], [6,2,-6]])
        M3 = ncon([U0,T0,U0,T1,U1,U1],[[-1,3,4],   [3,-4,1,2],  [-3,1,-7], [-5,6,4,5], [2,5,-8],  [-2,6,-6]])
        M4 = ncon([U0,T0,U0,T1,U1,U1],[[5,-1,3],   [5,4,6,-2],  [6,-3,-7], [-5,2,3,1], [-4,1,-8], [4,2,-6]])

        # print("M shape:\n",M.shape)
        dim = M1.size()
        M1 = M1.flatten().reshape(dim[0] * dim[1] * dim[2] * dim[3], dim[4] * dim[5] * dim[6] * dim[7])
        M2 = M2.flatten().reshape(dim[0] * dim[1] * dim[2] * dim[3], dim[4] * dim[5] * dim[6] * dim[7])
        M3 = M3.flatten().reshape(dim[0] * dim[1] * dim[2] * dim[3], dim[4] * dim[5] * dim[6] * dim[7])
        M4 = M4.flatten().reshape(dim[0] * dim[1] * dim[2] * dim[3], dim[4] * dim[5] * dim[6] * dim[7])
        M = 1/4 * (M1 + M2 + M3 + M4)
        # print("M shape:\n",M.shape)
        # print("M:\n",M)
        # M_np = M.detach().numpy()
        vec, ei, _ = power(M)
        # ei_s,vec_s = eigs(M_np, k=1, which='LM')
        # ei_s = torch.real(torch.from_numpy(ei_s)) #肯定是real的
        # vec_s = torch.real(torch.from_numpy(vec_s))
        print("eigmax_p             :", ei.item())
        Env = vec.flatten().reshape(dim[0], dim[1], dim[2], dim[3])
    ...
    def Uforward(self, U):
        ...
        for i in range(self.RGstep):
            if i < self.RGstep - 2:
                ...
                U_.append(gaugeU)
                T1, Coef = self.contraction(T0, gaugeU)
                # construc the real T1
                T1 = T1 * Coef
                T0 = self.rotate(T1)

        Env = self.calc_transferM(U_[0],U_[1],T_[0],T_[1])
        # print("Env:\n",Env)
        EnvT = ncon([Env, T_[0]],[[1,2,3,4],[1,2,3,4]])
        # print("EnvT:\n",EnvT)
        delta = -self.Temp * torch.log(abs(EnvT)) / 2**(self.RGstep)
        f = fe + delta
...
if __name__ == '__main__':
    ...
    params = {
        'J': -1,
        'D': 3, # right now the D <= 4 are required, 0419
        'kB': 1,
        'Temp': 2.5,
        'RGstep': 30,
        'Torder': [3, 0, 1, 2],
    ...
    print(err_np)
    # np.save('dtrgB'+str(dtrg.D)+'.npy', err_np)
